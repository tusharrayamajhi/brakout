...
export class OrderAgent {
    ...
    constructor(
        ...
        this.prompt = promptService.OrderAgentPrompt()
        this.outputParser = outputParserService.getOrderAgentOutputParser()
    }


    @OnEvent('order_taking_agent')
    async messageAgent(data: {
        ...
        if (!business) return
        const OrderAgent = this.prompt.pipe(this.Model).pipe(this.outputParser)
        const response: any = await OrderAgent.invoke({
